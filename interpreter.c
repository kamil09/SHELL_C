...
void wykonajPolecenie(int argcT, char *argvT[100]){
	...
	signal(SIGSTOP, obsluga_CTRL_Z);
//	signal(SIGUSR1, obsluga_CTRL_C);
		...
		if( (line != NULL) && (*line)){
			...
			for(i = 0; i < iloscLini ; i++){
				...
				for(k = 0 ; k<iloscRownoleglych; k++ ){	
					...
					if( (argv[0]!=0) && (argv[0][0]>30 ) ) wykonajPolecenie(argc,argv);			//WYKONUJE POLECENIE
				}
